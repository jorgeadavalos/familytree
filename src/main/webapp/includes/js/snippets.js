...
function getNodeText(node) {
	...
	if (text == null || text.indexOf(_BEANSUFFIX) == -1) {
		text = node.value;
		if (typeof text != 'number')
			if (text == null || text.indexOf(_BEANSUFFIX) == -1) text = node.nodeValue;
		if (text == null || typeof text == 'number' || text.indexOf(_BEANSUFFIX) == -1) return null;
	}
	return text;
}
...
function getBeanName(node) {
	...
	const wrkArray = text.split(_BEANSUFFIX);
...
function populateFromJsonObjs(beans) {
	...
	for (let element of fields) {
		...
		var patterns = nodeText.split(_BEANSUFFIX);
		for (pattern of patterns) {
			...
			for (var j=0,jlen=keys.length;j<jlen;j++) {
				...
				setNodeText(element,value,_BEANSUFFIX+pattern+_beanSuffix,readonly);
			}
		}
	}
}
function populateFromServerJson(serverJson) {
	...
	for (let element of fields) {
		...
		var items = pattern.replace(_BEANSUFFIX,"").replace(_beanSuffix,"").split(".");
